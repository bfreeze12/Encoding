...
def display_huffman_stages(stages):
    try:
        ...
        txt_stages.config(state='normal')  # Enable editing
        txt_stages.delete('1.0', tk.END)  # Clear previous content
...
def display_encoding_table_pandas_gui(encoding_df):
    try:
        txt_encoding_table.config(state='normal')  # Enable editing
        txt_encoding_table.delete('1.0', tk.END)  # Clear previous content
        ...
        txt_encoding_table.config(state='disabled')  # Disable editing
    except Exception as e:
        messagebox.showerror("Error", f"Failed to display encoding table:\n{e}")

def display_efficiency(entropy, avg_length, efficiency):
    try:
        txt_efficiency.config(state='normal')  # Enable editing
        txt_efficiency.delete('1.0', tk.END)  # Clear previous content
        ...
        txt_efficiency.config(state='disabled')  # Disable editing
    except Exception as e:
...
window.geometry("900x800")  # Adjusted size for better display
